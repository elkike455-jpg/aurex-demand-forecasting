...
def _evaluate_one(series_id: str, benchmark_label: str, out_dir: Path) -> Dict[str, object]:
    ...
    ax.set_title(f"Deep-learning-time-series LSTM real vs predicted: {series_id}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Sales")
    ax.legend()
    fig.autofmt_xdate()
...
def main() -> None:
    out_dir = Path("reports") / "gnn_benchmarks" / "deep_learning_time_series_repo_lstm_impl3"
