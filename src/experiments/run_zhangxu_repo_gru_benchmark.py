...
from src.models.zhangxu_repo_gru import ZhangxuRepoGRUForecastModel
...
def _evaluate_one(series_id: str, benchmark_label: str, out_dir: Path) -> Dict[str, object]:
    ...
    model = ZhangxuRepoGRUForecastModel(
    ...
    training_row = {
        ...
        "benchmark_label": benchmark_label,
        "model": "ZhangxuRepoGRU",
        "epochs": int(model.training_history_.get("epochs", 0)),
        "final_loss": float(model.training_history_.get("final_loss", np.nan)),
        "context_length": OFFICIAL_BENCHMARK_PROTOCOL.context_length,
        "train_days": int(len(y_train)),
        "fit_days": int(len(y_fit)),
    ...
    ax.set_title(f"zhangxu repo GRU real vs predicted: {series_id}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Sales")
    ax.legend()
    fig.autofmt_xdate()
    ...
    print(
        f"[RUN] {series_id} | loss={metrics_row['final_loss']:.6f} | mae={metrics_row['mae']:.6f} | "
