...
def _evaluate_one(series_id: str, benchmark_label: str, out_dir: Path) -> Dict[str, object]:
    ...
    model = SARIMAXModel().fit(data["y_fit"], data["X_fit"])
    y_pred, conf_low, conf_up = model.forecast(len(data["y_test"]), data["X_test"])
    ...
    training_row = {
        ...
        "benchmark_label": benchmark_label,
        "model": "SARIMAX",
        "best_order": str(model.best_order),
        "best_seasonal_order": str(model.best_seasonal_order),
        "upper_cap": float(model.upper_cap) if model.upper_cap is not None else float("nan"),
        "zero_rate_fit": float(model.zero_rate) if model.zero_rate is not None else float("nan"),
        "final_loss": float("nan"),
        "context_length": OFFICIAL_BENCHMARK_PROTOCOL.context_length,
        "train_days": int(len(data["y_train"])),
        "fit_days": int(len(data["y_fit"])),
    ...
    ax.set_title(f"SARIMAX real vs predicted: {series_id}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Sales")
    ax.legend()
    fig.autofmt_xdate()
    ...
    print(
        f"[RUN] {series_id} | order={model.best_order} | seasonal={model.best_seasonal_order} | "
