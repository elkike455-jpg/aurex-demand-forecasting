...
from src.models.pytorch_forecasting_deepar import (
    DeepARBenchmarkConfig,
    build_single_series_frame,
    fit_deepar_model,
    forecast_with_deepar,
    make_deepar_datasets,
...
def _evaluate_one(series_id: str, benchmark_label: str, out_dir: Path) -> Dict[str, object]:
    ...
    train_dataset, _ = make_deepar_datasets(train_df, fit_df, config)
    val_predict_dataset = make_predict_dataset(train_dataset, fit_df)
    _, phase1 = fit_deepar_model(train_dataset, val_predict_dataset, config, max_epochs=config.max_epochs)
    ...
    ax.set_title(f"PyTorch Forecasting DeepAR real vs predicted: {series_id}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Sales")
    ax.legend()
    fig.autofmt_xdate()
