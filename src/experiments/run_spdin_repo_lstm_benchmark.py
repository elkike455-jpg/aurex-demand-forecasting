...
from src.models.spdin_repo_lstm import SpdinRepoLSTMForecastModel
...
def _evaluate_one(series_id: str, benchmark_label: str, out_dir: Path) -> Dict[str, object]:
    df = load_m5_single_series(base_path="data/raw/m5", random_pick=False, series_id=series_id)
    ...
    model = SpdinRepoLSTMForecastModel(
    ...
    ax.set_title(f"spdin repo LSTM real vs predicted: {series_id}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Sales")
    ax.legend()
    fig.autofmt_xdate()
...
def main() -> None:
    out_dir = Path("reports") / "gnn_benchmarks" / "spdin_repo_lstm_impl4"
